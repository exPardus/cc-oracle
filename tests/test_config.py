...
def test_config_path_uses_state_dir_logic(monkeypatch, tmp_path):
    ...
    assert p.parent.parent == tmp_path / "plugin-data"
...
def test_added_marker_triggers_block(monkeypatch, tmp_path):
    _isolate(monkeypatch, tmp_path)
    _write_config(tmp_path, {"markers": {"add": ["going in circles"]}})
    payload = _payload(tmp_path, [_user_prompt("x"), _assistant_text("I keep going in circles here.")])
